...
- Pick formatter by this order: if flake defines `formatter` output, use `nix fmt`; otherwise default to `nixfmt` (`nix run nixpkgs#nixfmt -- .`). `nixfmt-rfc-style` is now an alias for `nixfmt`; use `nixfmt` directly. Exception: repo already formatted with different tool and no `formatter` output, match its existing style instead of reformatting, since `nixfmt`, `alejandra`, and `nixpkgs-fmt` produce incompatible output. Confirm guessed formatter with `--check` (already-formatted file exits 0) before writing.
